...
<head>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #1a1a2e; color: #e0e0e0; padding: 24px; line-height: 1.6; }
    .card { background: #16213e; border-radius: 12px; overflow: hidden; box-shadow: 0 8px 32px rgba(0,0,0,0.3); max-width: 900px; margin: 0 auto; }
    .header { background: linear-gradient(135deg, #e53935, #b71c1c); padding: 20px 24px; display: flex; align-items: center; gap: 12px; }
    .header h2 { color: white; font-size: 18px; font-weight: 600; }
    .header .icon { font-size: 24px; }
    .body { padding: 24px; }
    .section { margin-bottom: 20px; }
    .section-title { font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; color: #90a4ae; font-weight: 700; margin-bottom: 8px; }
    .failure-box { background: #2d1b1b; border: 1px solid #5c2020; border-radius: 8px; padding: 14px 16px; font-family: 'Fira Code', monospace; font-size: 12px; color: #ef9a9a; white-space: pre-wrap; word-break: break-word; max-height: 200px; overflow-y: auto; }
    .context-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
    .context-item { background: #1a2744; border-radius: 8px; padding: 12px 14px; }
    .context-label { font-size: 10px; text-transform: uppercase; color: #78909c; margin-bottom: 4px; }
    .context-value { font-size: 13px; color: #b0bec5; word-break: break-all; }
    .analysis-box { background: #1a2744; border-radius: 8px; padding: 16px; font-size: 13px; color: #cfd8dc; line-height: 1.7; }
    .analysis-box p { margin-bottom: 10px; }
    .analysis-box ul, .analysis-box ol { margin: 8px 0 8px 20px; }
    .analysis-box li { margin-bottom: 6px; }
    .analysis-box strong, .analysis-box b { color: #fff; }
    .suggestions-list { list-style: none; padding: 0; }
    .suggestion { background: #1a2744; border-radius: 8px; padding: 12px 16px; margin-bottom: 8px; display: flex; align-items: flex-start; gap: 10px; }
    .suggestion-num { background: #ff6f00; color: white; width: 22px; height: 22px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; flex-shrink: 0; }
    .suggestion-text { font-size: 13px; color: #b0bec5; }
    .footer { padding: 14px 24px; background: #0f1a2e; text-align: center; font-size: 11px; color: #546e7a; }
  </style>
</head>
<body>
  <div class="card">
    ...
    <div class="body">
      <div class="section">
        ...
        <div class="failure-box">${(step.result?.message || 'Unknown error').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
      </div>
      <div class="section">
        <div class="section-title">Page Context</div>
        <div class="context-grid">
          <div class="context-item">
            <div class="context-label">URL</div>
            <div class="context-value">${failureContext.pageUrl}</div>
          </div>
          <div class="context-item">
            <div class="context-label">Title</div>
            <div class="context-value">${failureContext.pageTitle}</div>
          </div>
        </div>
      </div>
      <div class="section">
        <div class="section-title">Root Cause Analysis</div>
        <div class="analysis-box">${analysis.split(/Suggested Fix/i)[0].trim().replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/^### (.+)$/gm, '<p><strong>$1</strong></p>').replace(/^## (.+)$/gm, '<p><strong>$1</strong></p>').replace(/^# (.+)$/gm, '<p><strong>$1</strong></p>').replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/\*(.+?)\*/g, '<em>$1</em>').replace(/`(.+?)`/g, '<code style="background:#263238;padding:2px 5px;border-radius:3px;font-size:12px;">$1</code>').replace(/^- (.+)$/gm, '<li>$1</li>').replace(/^(\d+)\. (.+)$/gm, '<li>$2</li>').replace(/\n\n/g, '</p><p>').replace(/\n/g, '<br>')}</div>
      </div>
      <div class="section">
        <div class="section-title">Suggested Fixes</div>
        <ol class="suggestions-list">
          ${suggestions.slice(0, 3).map((s, i) => `<div class="suggestion"><span class="suggestion-num">${i + 1}</span><span class="suggestion-text">${s.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/`(.+?)`/g, '<code>$1</code>')}</span></div>`).join('\n')}
        </ol>
      </div>
    </div>
    <div class="footer">Analysis generated by AI-powered RootCauseAnalyzer</div>
  </div>
</body>
</html>
`;
